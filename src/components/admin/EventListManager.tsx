...
const EventListManager: React.FC<EventListManagerProps> = ({
  ...
  const getTableName = () => {
    switch (listType) {
      case 'schedule': return 'event_schedule_items';
      case 'includes': return 'event_includes_items';
      case 'highlights': return 'event_highlights_items';
    }
  };
  ...
  if (!eventId) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-muted-foreground">Save the event first to manage {listType} items.</p>
        </CardContent>
      </Card>
    );
  }
